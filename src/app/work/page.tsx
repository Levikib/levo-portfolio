...
const PROJECTS = [
  {
    ...
    accent: "#7c3aed", accentLight: "rgba(124,58,237,0.08)", accentBorder: "rgba(124,58,237,0.2)",
    cardBg: "#ffffff",
    vizBg: "#0f0a1e",
  ...
  },
  {
    ...
    accent: "#d97706", accentLight: "rgba(217,119,6,0.07)", accentBorder: "rgba(217,119,6,0.2)",
    cardBg: "#faf7f0",
    vizBg: "#0f0c05",
  ...
  },
  {
    ...
    accent: "#e11d48", accentLight: "rgba(225,29,72,0.07)", accentBorder: "rgba(225,29,72,0.2)",
    cardBg: "#ffffff",
    vizBg: "#0f050a",
  ...
  },
  {
    ...
    accent: "#059669", accentLight: "rgba(5,150,105,0.07)", accentBorder: "rgba(5,150,105,0.2)",
    cardBg: "#faf7f0",
    vizBg: "#030f09",
    ...
    <div style={{ padding:"24px", height:"100%", display:"flex", flexDirection:"column", gap:"16px" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
        <div>
          ...
          <div style={{ fontFamily:"var(--font-display)", fontWeight:800, fontSize:"20px", color:"white", lineHeight:1, marginTop:"4px" }}>KSH 1,521,400</div>
      ...
      <div>
        ...
        <div style={{ display:"flex", alignItems:"flex-end", gap:"4px", height:"70px" }}>
          {payments.map((v,i)=>(
            <div key={i} style={{ flex:1, height:`${(v/maxP)*66}px`, background: i===payments.length-1 ? `linear-gradient(to top, ${accent}, ${accent}cc)` : `${accent}${Math.round(20+(v/maxP)*50).toString(16).padStart(2,'0')}`, borderRadius:"2px 2px 0 0", boxShadow:i===payments.length-1?`0 0 10px ${accent}60`:"none", transition:"height 0.6s" }} />
          ))}
        </div>
      ...
      <div>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:"2px" }}>
          {Array.from({length:36}).map((_,i)=>(
            <div key={i} style={{ width:"12px", height:"12px", borderRadius:"2px", background: i<34 ? i%9===0?`${accent}cc`:"#4ead6a50" : "rgba(255,255,255,0.05)" }} />
          ))}
          <div style={{ fontFamily:"var(--font-mono)", fontSize:"7px", color:"rgba(255,255,255,0.18)", alignSelf:"center", marginLeft:"3px" }}>+134</div>
        </div>
      </div>
      {/* Bottom stats */}
      <div style={{ display:"flex", gap:"6px", marginTop:"auto" }}>
        {[{l:"Tenants",v:"247",c:accent},{l:"Bills Due",v:"18",c:"#d97706"},{l:"Paid",v:"11",c:"#4ead6a"}].map(s=>(
          <div key={s.l} style={{ flex:1, background:"rgba(255,255,255,0.04)", borderRadius:"4px", padding:"7px 8px", border:`1px solid ${s.c}20` }}>
            <div style={{ fontFamily:"var(--font-display)", fontWeight:800, fontSize:"16px", color:s.c, lineHeight:1 }}>{s.v}</div>
...
function NseViz({ accent }: { accent: string }) {
  ...
  return (
    <div style={{ padding:"24px", height:"100%", display:"flex", flexDirection:"column", gap:"12px" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
        <div>
          ...
          <div style={{ display:"flex", alignItems:"baseline", gap:"8px", marginTop:"3px" }}>
            <div style={{ fontFamily:"var(--font-display)", fontWeight:800, fontSize:"24px", color:"white" }}>{price.toFixed(2)}</div>
      ...
      <canvas ref={canvasRef} width={400} height={120} style={{ width:"100%", height:"120px" }} />
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"6px" }}>
        {[{n:"RSI",v:"58.4",s:"Neutral",c:accent},{n:"MACD",v:"+1.23",s:"Bullish",c:"#4ead6a"},{n:"Vol",v:"2.4M",s:"Above avg",c:"#a855f7"}].map(ind=>(
          <div key={ind.n} style={{ background:"rgba(255,255,255,0.04)", borderRadius:"4px", padding:"8px", border:`1px solid ${ind.c}20` }}>
            <div style={{ fontFamily:"var(--font-mono)", fontSize:"7px", color:"rgba(255,255,255,0.25)", letterSpacing:"0.1em" }}>{ind.n}</div>
            <div style={{ fontFamily:"var(--font-display)", fontWeight:800, fontSize:"15px", color:ind.c, marginTop:"2px" }}>{ind.v}</div>
            <div style={{ fontFamily:"var(--font-mono)", fontSize:"7px", color:"rgba(255,255,255,0.2)", marginTop:"1px" }}>{ind.s}</div>
          </div>
        ))}
      </div>
      <div style={{ background:`${accent}12`, border:`1px solid ${accent}25`, borderRadius:"4px", padding:"10px 12px", display:"flex", gap:"8px" }}>
...
function ShantechViz({ accent }: { accent: string }) {
  ...
  return (
    <div ref={ref} style={{ padding:"24px", height:"100%", display:"flex", flexDirection:"column", gap:"14px" }}>
      <div>
        ...
        <div style={{ fontFamily:"var(--font-display)", fontWeight:800, fontSize:"20px", color:"white", marginTop:"4px" }}>250K+ Engagements</div>
      </div>
      {funnel.map((f,i)=>(
        <div key={f.l}>
      ...
      <div style={{ marginTop:"4px" }}>
        {[{n:"Hotel Digital Transform",r:"Web + Social"},{n:"Retail Fashion Brand",r:"Meta Ads + CRM"},{n:"Real Estate Co.",r:"Full-Stack Web"},{n:"F&B Brand",r:"Social Strategy"}].map((c,i)=>(
          <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"6px 0", borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
            <span style={{ fontFamily:"var(--font-body)", fontSize:"10px", color:"rgba(255,255,255,0.5)" }}>{c.n}</span>
  ...
  return (
    <div style={{ padding:"24px", height:"100%", display:"flex", flexDirection:"column", gap:"14px" }}>
      <div>
        ...
        <div style={{ fontFamily:"var(--font-display)", fontWeight:800, fontSize:"20px", color:"white", marginTop:"4px" }}>72 Pages Solo</div>
      </div>
      ...
      <div>
        ...
        <div style={{ display:"flex", gap:"2px", marginBottom:"3px" }}>{p1.map(c=><div key={c} style={{ flex:1, height:"16px", background:c, borderRadius:"1px" }} />)}</div>
        <div style={{ display:"flex", gap:"2px" }}>{p2.map(c=><div key={c} style={{ flex:1, height:"16px", background:c, borderRadius:"1px" }} />)}</div>
      </div>
    </div>
  );
}
    ...
    <div style={{ background:"var(--bg)", minHeight:"100vh" }}>
      ...
      <div style={{ background:"var(--bg)", paddingTop:"140px", paddingBottom:"72px", paddingLeft:"48px", paddingRight:"48px", borderBottom:"1px solid var(--border)" }}>
        ...
        <div style={{ display:"flex", gap:"6px", flexWrap:"wrap" }}>
          {CATS.map(c=>(
            <button key={c} onClick={()=>setActive(c)} style={{ fontFamily:"var(--font-mono)", fontSize:"10px", letterSpacing:"0.15em", textTransform:"uppercase", padding:"8px 18px", background:active===c?"var(--purple)":"transparent", border:`1px solid ${active===c?"var(--purple)":"var(--border)"}`, color:active===c?"white":"var(--text-3)", cursor:"pointer", transition:"all 0.2s", borderRadius:"2px" }}>{c}</button>
          ))}
        </div>
      </div>
      ...
      <div>
        {filtered.map((p, idx) => {
          ...
          return (
            <div key={p.id} style={{
              background: isEven ? "#ffffff" : "var(--bg-2)",
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", minHeight:"380px" }}>
                ...
                {/* ── LEFT — always: identity + details ── */}
                <div style={{ padding:"40px 44px", display:"flex", flexDirection:"column", justifyContent:"space-between", borderRight:"1px solid var(--border)" }}>
                  <div>
                    {/* Number watermark */}
                    <div style={{ fontFamily:"var(--font-display)", fontWeight:800, fontSize:"72px", color:`${p.accent}10`, lineHeight:1, marginBottom:"-20px", letterSpacing:"-0.05em" }}>{p.num}</div>
                    ...
                    <h2 style={{ fontFamily:"var(--font-display)", fontWeight:800, fontSize:"clamp(26px,3vw,40px)", letterSpacing:"-0.02em", color:"var(--text)", lineHeight:1, marginBottom:"6px" }}>{p.name}</h2>
                    <div style={{ fontFamily:"var(--font-body)", fontSize:"13px", color:p.accent, marginBottom:"18px", fontStyle:"italic" }}>{p.tagline}</div>
                    <p style={{ fontFamily:"var(--font-body)", fontSize:"13px", color:"var(--text-3)", lineHeight:1.85, marginBottom:"20px", maxWidth:"480px" }}>{p.desc}</p>

                    {/* Tags */}
                    <div style={{ display:"flex", flexWrap:"wrap", gap:"5px" }}>
                      {p.tags.map(t=>(
                        <span key={t} style={{ fontFamily:"var(--font-mono)", fontSize:"9px", letterSpacing:"0.05em", color:"var(--text-3)", background:"var(--bg-2)", border:"1px solid var(--border)", padding:"4px 9px" }}>{t}</span>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ marginTop:"28px" }}>
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"8px", marginBottom:"20px" }}>
                      {p.metrics.map(m=>(
                        <div key={m.label} style={{ background:`${p.accent}08`, border:`1px solid ${p.accent}20`, padding:"10px 8px", textAlign:"center" }}>
                          <div style={{ fontFamily:"var(--font-display)", fontWeight:800, fontSize:"14px", color:p.accent, lineHeight:1, marginBottom:"3px" }}>{m.val}</div>
                ...
                {/* ── RIGHT — always: dark viz panel ── */}
                <div style={{ background:p.vizBg, position:"relative", overflow:"hidden", minHeight:"380px" }}>
                  {/* Subtle dot grid on viz */}
                  <div style={{ position:"absolute", inset:0, backgroundImage:`radial-gradient(${p.accent}15 1px, transparent 1px)`, backgroundSize:"20px 20px", opacity:0.6, pointerEvents:"none" }} />
                  {/* Corner glow */}
                  <div style={{ position:"absolute", top:"-40px", right:"-40px", width:"200px", height:"200px", background:`radial-gradient(circle, ${p.accent}15, transparent 70%)`, filter:"blur(30px)", pointerEvents:"none" }} />
              ...
              {/* ── DEEP DIVE EXPANSION ── */}
              {isExpanded && (
                <div style={{ background:`${p.accent}06`, borderTop:`1px solid ${p.accent}20`, padding:"36px 44px", display:"grid", gridTemplateColumns:"1fr 1fr", gap:"48px" }}>
                  <div>
