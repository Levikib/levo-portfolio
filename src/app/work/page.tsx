...
const PROJECTS = [
  {
    num: "01", id: "makeja", tier: "featured" as const,
    name: "Makeja Homes",
    ...
    vizBg: "#0f0a1e",
    screenshot: "/work/makeja-screenshot.png",
    ...
    headline: { val: "KSH 1.5M", label: "flowing through the platform every month" },
    github: "https://github.com/Levikib/makeja-homes",
    live: "https://makejahomes.co.ke",
  },
  {
    num: "02", id: "ghostnet", tier: "featured" as const,
    name: "GhostNet",
    ...
    vizBg: "#020f08",
    screenshot: "/work/ghostnet-screenshot.png",
    desc: "Full-stack cybersecurity research & training platform. 13 modules, each with a concept page and an interactive lab, 243 guided lab steps, a ~5,450 XP economy with a 5-tier rank system, 9 standalone security tools, a live leaderboard, and GHOST Agent, a Groq llama-3.3-70b assistant embedded inside the platform. Cinematic matrix-rain splash on first entry, built with canvas and CSS, no animation library.",
    ...
    solution: "Module concept pages are open to browse with no signup wall; labs, progress, leaderboard, and GHOST Agent require a free Supabase-authenticated account. Supabase realtime plus polling for XP updates and the leaderboard. Groq llama-3.3-70b powering GHOST Agent for contextual guidance. 9 standalone tools: CVE feed, tool reference, research terminal, payload generator, blockchain tracer, CTF toolkit, AI report generator, MITRE ATT&CK visualizer, Shodan query builder. Custom matrix-rain and scramble-text welcome sequence in canvas/CSS.",
    tags: ["Next.js 14", "TypeScript", "Supabase", "Groq llama-3.3-70b", "Realtime", "Canvas API"],
    metrics: [{ val:"13", label:"Modules" },{ val:"243", label:"Lab Steps" },{ val:"5,450", label:"XP Economy" },{ val:"9", label:"Tools" }],
    headline: { val: "243", label: "guided lab steps across 13 modules" },
    github: "https://github.com/Levikib/ghostnet",
    live: "https://ghostnet-pi.vercel.app",
  },
  {
    num: "03", id: "hookah", tier: "secondary" as const,
    name: "Hookah Rental & Experience",
    tagline: "A 3D showroom for a lounge business.",
    category: "Engineering",
    status: "live", statusLabel: "Live in Production",
    year: "2026", type: "3D E-commerce Platform",
    accent: "#9d5cf5", accentBorder: "rgba(157,92,245,0.2)",
    vizBg: "#05030a",
    screenshot: "/work/hookah-screenshot.png",
    desc: "Full-stack 3D rental and booking platform for a premium hookah lounge business. A Meshy-generated GLB hookah model, Draco-compressed, sits at the center of a scroll-driven hero with mouse-parallax rotation, smoke and ember particles, and a GPU explode shader that disassembles the piece into its 7 parts as you scroll.",
    challenge: "Rental businesses sell a physical object and an experience. How do you make a product page feel like walking into the lounge itself, before the user has even booked?",
    solution: "React Three Fiber and Three.js render the hero model and disassembly sequence; GSAP ScrollTrigger pins and drives the scroll choreography. An orbiting 3D flavour wall showcases 25 flavours with category filtering. Full checkout runs through Paystack (initialize, redirect verify, HMAC-validated webhook), backed by a Supabase database and a complete admin dashboard for bookings, orders, inventory, and analytics.",
    tags: ["Next.js 16", "React Three Fiber", "Three.js", "GSAP", "Zustand", "Supabase", "Paystack"],
    metrics: [{ val:"3D", label:"Hero Model" },{ val:"25", label:"Flavours" },{ val:"7", label:"Disassembly Parts" },{ val:"Live", label:"Payments" }],
    github: "https://github.com/Levikib/hookah-website",
    live: "https://hookah-website-two.vercel.app",
  },
  {
    num: "04", id: "nse", tier: "secondary" as const,
  ...
  {
    num: "05", id: "shantech", tier: "secondary" as const,
  ...
  {
    num: "06", id: "chillminds", tier: "secondary" as const,
...
function ScreenshotViz({ src, alt, accent }: { src: string; alt: string; accent: string }) {
  return (
    <div style={{ height:"100%", display:"flex", alignItems:"center", justifyContent:"center", padding:"clamp(20px,3vw,32px)" }}>
      <div style={{ width:"100%", borderRadius:"8px", overflow:"hidden", border:`1px solid ${accent}30`, boxShadow:"0 20px 60px rgba(0,0,0,0.45)", background:"#0a0a0f" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"6px", padding:"9px 12px", background:"#151318", borderBottom:`1px solid ${accent}20` }}>
          {["#e11d48","#d97706","#4ead6a"].map(c => <span key={c} style={{ width:"7px", height:"7px", borderRadius:"50%", background:c, opacity:0.7 }} />)}
        </div>
        <div style={{ position:"relative", width:"100%", aspectRatio:"16/9" }}>
          <Image src={src} alt={alt} fill sizes="(max-width: 700px) 100vw, 50vw" style={{ objectFit:"cover", objectPosition:"top" }} />
        </div>
      </div>
    </div>
  );
...
const VIZMAP: Record<string, React.FC<{ accent: string }>> = {
  nse: NseViz, shantech: ShantechViz, chillminds: ChillMindsViz,
};
...
function ProjectCard({ p, idx, expanded, onToggle }: {
  ...
  const headline = "headline" in p ? p.headline : null;
  const detailsPanel = (
    <div style={{ padding: isFeatured ? "clamp(24px,4vw,44px) clamp(20px,4vw,48px)" : "clamp(20px,3vw,32px)", display:"flex", flexDirection:"column", justifyContent:"space-between", background: isEven ? "#ffffff" : "var(--bg-2)" }}>
      <div>
        ...
        <h2 style={{ fontFamily:"var(--font-syne)", fontWeight:800, fontSize: isFeatured ? "clamp(28px,3vw,42px)" : "clamp(22px,2.4vw,28px)", letterSpacing:"-0.02em", color:"var(--text)", lineHeight:1, marginBottom:"6px" }}>{p.name}</h2>
        <div style={{ fontFamily:"var(--font-dm-sans)", fontSize:"13px", color:p.accent, marginBottom:"16px", fontStyle:"italic" }}>{p.tagline}</div>
        <p style={{ fontFamily:"var(--font-dm-sans)", fontSize:"13px", color:"var(--text-3)", lineHeight:1.85, marginBottom:"18px", maxWidth:"500px" }}>{p.desc}</p>
        <div style={{ display:"flex", flexWrap:"wrap", gap:"5px" }}>
          {p.tags.slice(0, isFeatured ? p.tags.length : 4).map(t => (
      ...
      <div style={{ marginTop:"24px" }}>
        {headline && (
          <div style={{ display:"flex", alignItems:"baseline", gap:"10px", marginBottom:"18px", paddingBottom:"18px", borderBottom:"1px solid var(--border)" }}>
            <span style={{ fontFamily:"var(--font-syne)", fontWeight:800, fontSize:"28px", color:p.accent, lineHeight:1 }}>{headline.val}</span>
            <span style={{ fontFamily:"var(--font-dm-sans)", fontSize:"12px", color:"var(--text-3)", lineHeight:1.4 }}>{headline.label}</span>
          </div>
        )}
  ...
  const vizPanel = "screenshot" in p && p.screenshot ? (
    <div style={{ background:p.vizBg, position:"relative", overflow:"hidden", height:"100%" }}>
      <ScreenshotViz src={p.screenshot} alt={`${p.name} product screenshot`} accent={p.accent} />
    </div>
  ) : (
    <div style={{ background:p.vizBg, position:"relative", overflow:"hidden", minHeight: isFeatured ? "340px" : "260px" }}>
      ...
      {Viz && <div style={{ position:"absolute", inset:0, zIndex:1 }}><Viz accent={p.accent} /></div>}
    </div>
  );
  return (
    <div style={{ border:"1px solid var(--border)", overflow:"hidden", boxShadow: isFeatured ? "0 2px 24px rgba(0,0,0,0.06)" : "0 1px 8px rgba(0,0,0,0.04)" }}>
      <div style={{ height:"3px", background:`linear-gradient(90deg, ${p.accent}, ${p.accent}40, transparent)` }} />
      <div className={isFeatured ? "proj-main-grid" : "proj-main-grid proj-main-grid--compact"} style={{ display:"grid" }}>
      ...
      {expanded && (
        <div className="proj-deep-grid" style={{ background:`${p.accent}06`, borderTop:`1px solid ${p.accent}20`, padding:"clamp(20px,3vw,36px) clamp(20px,4vw,48px)", display:"grid", gap:"clamp(20px,3vw,52px)" }}>
          <div>
            ...
            <p style={{ fontFamily:"var(--font-dm-sans)", fontSize:"13px", color:"var(--text-3)", lineHeight:1.9, fontStyle:"italic", borderLeft:`2px solid ${p.accent}40`, paddingLeft:"16px", marginBottom:"20px" }}>{p.challenge}</p>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:"8px", alignContent:"start" }}>
            {p.metrics.map(m => (
              <div key={m.label} style={{ background:`${p.accent}08`, border:`1px solid ${p.accent}20`, padding:"14px 10px", textAlign:"center" }}>
                <div style={{ fontFamily:"var(--font-syne)", fontWeight:800, fontSize:"17px", color:p.accent, lineHeight:1, marginBottom:"5px", whiteSpace:"nowrap" }}>{m.val}</div>
                <div style={{ fontFamily:"var(--font-syne-mono)", fontSize:"8px", color:"var(--text-4)", letterSpacing:"0.1em", textTransform:"uppercase", whiteSpace:"nowrap" }}>{m.label}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    <div style={{ background:"var(--bg)", minHeight:"100vh" }}>
      <style>{`
        ...
        @media (min-width: 700px) {
          ...
          .proj-main-grid--compact { min-height: auto !important; }
      ...
      <div style={{ background:"var(--bg)", paddingTop:"clamp(96px,12vw,140px)", paddingBottom:"0", paddingLeft:"clamp(20px,5vw,48px)", paddingRight:"clamp(20px,5vw,48px)", borderBottom:"1px solid var(--border)" }}>
        ...
        <p style={{ fontFamily:"var(--font-dm-sans)", fontSize:"15px", color:"var(--text-3)", maxWidth:"460px", lineHeight:1.8, marginBottom:"40px" }}>
          Two SaaS products in production, one agency track record, a few things built for the love of it.
        </p>
        {/* ── TOGGLE ── */}
        <div style={{ display:"flex", alignItems:"center", gap:"16px", paddingBottom:"40px", flexWrap:"wrap" }}>
          <div style={{ display:"inline-flex", background:"#0c0a07", border:"1px solid rgba(255,255,255,0.12)", borderRadius:"5px", padding:"5px", gap:"4px" }}>
            {([
              { id:"portfolio", label:"Portfolio", sub:"6 Projects",   accent:"#7c3aed", glow:"rgba(124,58,237,0.45)" },
      ...
      {tab === "portfolio" && (
        <>
          ...
          <div style={{ background:"var(--bg)", padding:`clamp(28px,5vw,56px) clamp(16px,4vw,48px) 80px` }}>
            {featured.length > 0 && (
              <div style={{ display:"flex", flexDirection:"column", gap:"clamp(28px,4vw,40px)", marginBottom: secondary.length > 0 ? "clamp(48px,7vw,72px)" : 0 }}>
                {featured.map((p, idx) => (
            ...
            {secondary.length > 0 && (
              <div>
                <div style={{ fontFamily:"var(--font-syne-mono)", fontSize:"10px", letterSpacing:"0.2em", color:"var(--text-4)", textTransform:"uppercase", marginBottom:"20px" }}>// Also Built</div>
